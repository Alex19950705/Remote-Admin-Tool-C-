...
namespace WaveLib
{
    ...
    public unsafe class Win32
    {
        ...
        public static extern MMRESULT waveOutOpen(ref IntPtr hWaveOut, int uDeviceID, WaveFormat lpFormat, DelegateWaveOutProc dwCallBack, int dwInstance, int dwFlags);

        [DllImport("winmm.dll")]
        public static extern MMRESULT waveInOpen(ref IntPtr hWaveIn, int deviceId, WaveFormat wfx, DelegateWaveInProc dwCallBack, int dwInstance, int dwFlags);

        [DllImport("winmm.dll", SetLastError = true, EntryPoint = "waveInOpen")]
        public static extern MMRESULT waveInOpen2(ref IntPtr hWaveIn, int deviceId, ref WaveFormat wfx, SafeWaitHandle callBackEvent, int dwInstance, int dwFlags);
