    ...
    public class VideoCaptureDevice : IVideoSource
    {
                ...
                if (crossbarVideoInputs == null)
                {
                    ...
                    if (crossbarVideoInputs == null)
                    {
                        if (!IsRunning)
                            ...
                            for (var i = 0; i < 500 && crossbarVideoInputs == null; i++)
                                Thread.Sleep(10);
                    }
                }
        ...
        public bool IsRunning
        {
            get
            {
                ...
                {
                    // check thread status
                    if (thread.Join(0) == false) return true;
                ...
                if (videoCapabilities == null)
                {
                    ...
                    if (videoCapabilities == null)
                    {
                        if (!IsRunning)
                            ...
                            for (var i = 0; i < 500 && videoCapabilities == null; i++)
                                Thread.Sleep(10);
                    }
                }
                ...
                if (snapshotCapabilities == null)
                {
                    ...
                    if (snapshotCapabilities == null)
                    {
                        if (!IsRunning)
                            ...
                            for (var i = 0; i < 500 && snapshotCapabilities == null; i++)
                                Thread.Sleep(10);
                    }
                }
        ...
        public void Start()
        {
            ...
            {
                // check source
                if (string.IsNullOrEmpty(deviceMoniker)) throw new ArgumentException("Video source is not specified.");
        ...
        public void DisplayCrossbarPropertyPage(IntPtr parentWindow)
        {
            lock (sync)
            {
                ...
                for (var i = 0; i < 500 && !isCrossbarAvailable.HasValue && IsRunning; i++) Thread.Sleep(10);
            ...
            lock (sync)
            {
                if (!isCrossbarAvailable.HasValue)
                {
                    if (!IsRunning)
                        ...
                        for (var i = 0; i < 500 && !isCrossbarAvailable.HasValue; i++)
                            Thread.Sleep(10);
                }
        ...
        public bool SetCameraProperty(CameraControlProperty property, int value, CameraControlFlags controlFlags)
        {
            ...
            lock (sync)
            {
                ...
                var hr = pCamControl.Set(property, value, controlFlags);
        ...
        /// <exception cref="NotSupportedException">The video source does not support camera control.</exception>
        /// 
        public bool GetCameraProperty(CameraControlProperty property, out int value,
            out CameraControlFlags controlFlags)
        {
            ...
            lock (sync)
            {
                ...
                var pCamControl = (IAMCameraControl) tempSourceObject;
                var hr = pCamControl.Get(property, out value, out controlFlags);
            ...
            out int stepSize, out int defaultValue, out CameraControlFlags controlFlags)
        {
            ...
            lock (sync)
            {
                ...
                var pCamControl = (IAMCameraControl) tempSourceObject;
                var hr = pCamControl.GetRange(property, out minValue, out maxValue, out stepSize, out defaultValue,
                    out controlFlags);
        ...
        private void WorkerThread(bool runGraph)
        {
            ...
            try
            {
                ...
                if (type == null) throw new ApplicationException("Failed creating capture graph builder");
                ...
                if (type == null) throw new ApplicationException("Failed creating filter graph");
                ...
                if (type == null) throw new ApplicationException("Failed creating sample grabber");
                ...
                captureGraph.FindInterface(FindDirection.UpstreamOnly, Guid.Empty, sourceBase, typeof(IAMCrossbar).GUID,
                    out crossbarObject);
                if (crossbarObject != null) crossbar = (IAMCrossbar) crossbarObject;
                ...
                if (videoControl != null)
                {
                    ...
                    {
                        videoControl.GetCaps(pinStillImage, out var caps);
                ...
                if (runGraph)
                {
                    ...
                    {
                        var vih = (VideoInfoHeader) Marshal.PtrToStructure(mediaType.FormatPtr,
                    ...
                    {
                        if (mediaEvent != null)
                            if (mediaEvent.GetEvent(out var code, out var p1, out var p2, 0) >= 0)
                            {
                        ...
                        if (needToDisplayPropertyPage)
                        {
                            ...
                            if (crossbar != null) crossbarVideoInput = GetCurrentCrossbarInput(crossbar);
                        }
            ...
            catch (Exception exception)
            {
                ...
                if (VideoSourceError != null) VideoSourceError(this, new VideoSourceErrorEventArgs(exception.Message));
            }
            finally
            {
                // release all objects
            ...
            if (PlayingFinished != null) PlayingFinished(this, reasonToStop);
        }
        ...
        private void SetResolution(IAMStreamConfig streamConfig, VideoCapabilities resolution)
        {
            ...
            for (var i = 0; i < capabilitiesCount; i++)
                try
                {
            ...
            Guid pinCategory, VideoCapabilities resolutionToSet, ref VideoCapabilities[] capabilities)
        {
            ...
            if (streamConfigObject != null)
            {
                ...
                if (streamConfig != null)
                {
                    ...
                    if (resolutionToSet != null) SetResolution(streamConfig, resolutionToSet);
                }
            }

            // if failed resolving capabilities, then just create empty capabilities array,
            // so we don't try again
            if (capabilities == null) capabilities = new VideoCapabilities[0];
        }
        ...
        private void DisplayPropertyPage(IntPtr parentWindow, object sourceObject)
        {
            try
            {
                ...
                pPropPages.GetPages(out var caGUID);
        ...
        private VideoInput[] ColletCrossbarVideoInputs(IAMCrossbar crossbar)
        {
            lock (cacheCrossbarVideoInputs)
            {
                if (cacheCrossbarVideoInputs.ContainsKey(deviceMoniker)) return cacheCrossbarVideoInputs[deviceMoniker];
                ...
                if (crossbar != null)
                {
                    ...
                    if (crossbar.get_PinCounts(out var outPinsCount, out var inPinsCount) == 0)
                        // collect all video inputs
                        for (var i = 0; i < inPinsCount; i++)
                        {
                            if (crossbar.get_CrossbarPinInfo(true, i, out var pinIndexRelated, out var type) != 0) continue;

                            if (type < PhysicalConnectorType.AudioTuner) videoInputsList.Add(new VideoInput(i, type));
                        }
                }
        ...
        private VideoInput GetCurrentCrossbarInput(IAMCrossbar crossbar)
        {
            ...
            if (crossbar.get_PinCounts(out var outPinsCount, out var inPinsCount) == 0)
            {
                var videoOutputPinIndex = -1;
                int pinIndexRelated;

                // find index of the video output pin
                for (var i = 0; i < outPinsCount; i++)
                {
                    if (crossbar.get_CrossbarPinInfo(false, i, out pinIndexRelated, out var type) != 0) continue;
                ...
                if (videoOutputPinIndex != -1)
                {
                    ...
                    if (crossbar.get_IsRoutedTo(videoOutputPinIndex, out var videoInputPinIndex) == 0)
                    {
                        crossbar.get_CrossbarPinInfo(true, videoInputPinIndex, out pinIndexRelated, out var inputType);
        ...
        private void SetCurrentCrossbarInput(IAMCrossbar crossbar, VideoInput videoInput)
        {
            if (videoInput.Type != PhysicalConnectorType.Default)
            {
                ...
                if (crossbar.get_PinCounts(out var outPinsCount, out var inPinsCount) == 0)
                {
                    ...
                    for (var i = 0; i < outPinsCount; i++)
                    {
                        if (crossbar.get_CrossbarPinInfo(false, i, out pinIndexRelated, out type) != 0) continue;
                    ...
                    for (var i = 0; i < inPinsCount; i++)
                    {
                        if (crossbar.get_CrossbarPinInfo(true, i, out pinIndexRelated, out type) != 0) continue;
        ...
        private void OnNewFrame(Bitmap image)
        {
            ...
            if (!stopEvent.WaitOne(0, false) && NewFrame != null) NewFrame(this, new NewFrameEventArgs(image));
        }
        ...
        {
            var timeSinceStarted = DateTime.Now - startTime;
